...
- **펭귄 아바타 꾸미기** (`/avatar`) — 32×32 도트(픽셀 아트) 펭귄을 몸 색 / 모자 / 안경 / 목 / 소품 레이어로 겹쳐 꾸밉니다. 옵션을 고르면 미리보기에 바로 반영되고, "랜덤" 버튼으로 무작위 조합을 만들 수 있습니다. 고른 조합은 `localStorage('penguin-look')`에 저장되어 새로고침해도 유지됩니다. 같은 커스터마이저를 등록 화면에서도 그대로 써서 프로필용 펭귄을 고릅니다. 홈 화면에 링크는 없고 주소로 직접 들어갑니다
...
- 옵션 목록은 `src/config/penguinLook.ts`의 `CATEGORIES`에 등록합니다
